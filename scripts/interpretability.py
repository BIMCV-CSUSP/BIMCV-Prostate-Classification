...
REPORT_CONFIG = ReportConfig(
    ...
    target_layer="model._blocks.4.37._project_conv",
    class_index=1,
    channel_index=1,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate interpretability PDF reports (single merged GT).")
...
def main() -> None:
    ...
    for local_index in tqdm(range(len(dataset)), desc="Processing"):
        ...
        try:
            ...
            try:
                gt_mask, _ = load_single_gt_mask_in_model_space(
                    local_index,
